...
2. **Replay confirmation.** For a finding classified VERIFIED on the basis of a reproducible effect, confirm from the audit log (APTS-AR-006) that the effect was observed on an out-of-band channel the discovering agent did not control, rather than inferred solely from the agent's own PoC output.
...
4. **Forged-submission resistance.** Confirm that a finding's integrity status is bound to evidence the discovering agent cannot author or alter after the fact (out-of-band observations and audit entries under APTS-AR-006), so that a finding cannot be marked VERIFIED on the strength of agent-supplied evidence alone.
